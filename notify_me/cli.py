...
@click.option(
    "--storage-type",
    type=click.Choice(list(map(lambda x: x.name, StorageType)), case_sensitive=True),
    default=storage_default,
    help="where to load and save our config from and to",
)
@click.option(
    "--log-level",
    type=click.Choice(["debug", "info", "error"], case_sensitive=False),
    default="info",
    help="what log level do we log at?",
)
